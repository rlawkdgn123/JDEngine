...
public:
    ...
    ParticleSystem(ID2D1DeviceContext7* ctx) {
        ...
        leafBitmap = AssetManager::Instance().GetTexture("leaf");//leaf
        footdustBitmap = AssetManager::Instance().GetTexture("dust");//footdust
        smokeBitmap = AssetManager::Instance().GetTexture("smoke");//smoke
        waveBitmap = AssetManager::Instance().GetTexture("wave");//wave
    }
    ...
    void SpawnSmokeParticle(const Vector2F& pos, int count = 6, float baseScale = 2.0f, float maxLife = 2.5f)
    {
        for (int i = 0; i < count; ++i) {
            ...
            p.pos.y += 30;
            p.vel = { RandomFloat(-10.f, 10.f), RandomFloat(10.f, 30.f) };
            p.life = maxLife;
    ...
    void RenderLeaves(ID2D1DeviceContext7* ctx, D2D1_MATRIX_3X2_F worldTransform)
    {
        if (!leafBitmap) return;
        ctx->SetPrimitiveBlend(D2D1_PRIMITIVE_BLEND_SOURCE_OVER);

        for (auto& p : particles) {
            // 1. 이 파티클만을 위한 변환 행렬을 계산합니다.
            // 크기(Scale)
            // 회전(Rotation)
            // 월드 위치로 이동(Translation) - GameObject처럼 Y 좌표의 부호를 뒤집어줍니다.
            auto S = D2D1::Matrix3x2F::Scale(p.imageScale, p.imageScale, { 0, 0 });
            auto R = D2D1::Matrix3x2F::Rotation(p.rotation, { 0, 0 });
            auto T = D2D1::Matrix3x2F::Translation(p.pos.x, -p.pos.y);

            // 파티클의 최종 월드 행렬 = 크기 * 회전 * 이동
            D2D1_MATRIX_3X2_F particleWorldMatrix = S * R * T;

            // 2. 최종 렌더링 행렬 = 파티클 월드 행렬 * 카메라 기본 변환
            //    (주의: Transform 클래스의 Y축 반전(-p.pos.y)은 카메라 파이프라인에 이미 포함되어 있으므로,
            //     여기서는 p.pos.y를 그대로 사용해야 합니다. baseTransform에 그 역할이 들어있습니다.)
            D2D1_MATRIX_3X2_F finalTransform = particleWorldMatrix * worldTransform;

            ctx->SetTransform(finalTransform);

            float half = p.size * 0.5f;
            D2D1_RECT_F dest = { -half, -half, half, half };
    ...
    void RenderSmoke(ID2D1DeviceContext7* ctx, D2D1_MATRIX_3X2_F worldTransform)
    {
        if (!smokeBitmap) return;
        ctx->SetPrimitiveBlend(D2D1_PRIMITIVE_BLEND_SOURCE_OVER);

        for (auto& p : particles) {
            auto S = D2D1::Matrix3x2F::Scale(p.imageScale, p.imageScale);
            auto R = D2D1::Matrix3x2F::Rotation(p.rotation);
            auto T = D2D1::Matrix3x2F::Translation(p.pos.x, -p.pos.y);
            D2D1_MATRIX_3X2_F particleWorldMatrix = S * R * T;
            D2D1_MATRIX_3X2_F finalTransform = particleWorldMatrix * worldTransform;
    ...
    void RenderWaterWave(ID2D1DeviceContext7* ctx, D2D1_MATRIX_3X2_F worldTransform)
    {
        ...
        for (auto& p : particles) {
            ...
            // 3. 파티클을 '원점' 기준으로 그립니다.
            float half = p.size * 0.5f; // imageScale은 행렬에 적용했으므로 여기서 제외
            D2D1_RECT_F dest = { -half, -half, half, half };
